...
class connect {
    ...
    public static function close() {
        self::$con = null;  // Cierra la conexión asignando null al objeto PDO
    }
}
...
<header>
    ...
    <div class="secciones-menu">
        ...
        <!-- Sección de Malteadas -->
        <section class="malteadas">
            <h2 class="malteadas-titulo">MALTEADAS</h2>
                <div class="container-malteadas">
                    <?php
                    $sql = "SELECT id, nombre, descripcion, precio FROM productos WHERE id_categoria=4";
                    $malteadas = connect::query($sql);
                    connect::close();
                    foreach($malteadas as $row){ ?>
        ...
        <section class="Tortas">
            <h2 class="Tortas-titulo">TORTAS</h2>
                <div class="container-tortas">
                
                    <?php
                    $sql = "SELECT id, nombre, descripcion, precio FROM productos WHERE id_categoria=1";
                    $tortas = connect::query($sql);
                    connect::close();
    ...
    <section class="Helados"></section>
            <h2 class="Helados-titulo">HELADOS</h2>
                <div class="container-helados">
                    <?php
                    $sql = "SELECT id, nombre, descripcion, precio FROM productos WHERE id_categoria=3 LIMIT 3";
                    $helados = connect::query($sql);
                    connect::close();
                    foreach($helados as $row){ ?>
                        <div class="imagen-texto">
                            <?php
                            $id = $row['id'];
                            $imagen = "img/productos/" . $id . "/img1.png";
        ...
        <section class="EnsaladadeFrutas"></section>
            <h2 class="Ensalada-titulo">ENSALADAS DE FRUTAS</h2>
                <div class="container-ensaladas">
                    <?php
                    $sql = "SELECT id, nombre, descripcion, precio FROM productos WHERE id_categoria=2";
                    $ensaladaFrutas = connect::query($sql);
                    connect::close();
                    foreach($ensaladaFrutas as $row){ ?>
                            <div class="imagen-texto">
                                <?php
                                $id = $row['id'];
                                $imagen = "img/productos/" . $id . "/img1.png";
